...
function showBook() {
  ...
  app.innerHTML = `<div class="edu-container edu-main book-page"><div class="book-head"><div><div class="home-kicker">NAVIANS</div><h2>ナビアン図鑑</h2><p class="edu-page-lead">出会ったナビアンを集めよう。 ${gotCount} / ${all.length}</p></div><button id="bookBack" class="edu-btn edu-btn-secondary">タイトル</button></div><div class="book-grid">${all.map(([file,name]) => { const count=Number(gets[file])||0; const evolved=file.includes('-evolved'); const folder=evolved?'zako-evolved':'zako'; const locked=count===0; return `<article class="navian-card ${count?'is-new':''} ${locked?'is-locked':''}"><div class="navian-card-img">${locked ? `<div class="navian-locked" aria-label="まだ出会っていない"><img src="${NAV_BASE}${folder}/${file}" alt="" aria-hidden="true"><span>？？？</span></div>` : `<img src="${NAV_BASE}${folder}/${file}" alt="${name}">`}</div><div class="navian-card-body"><span class="navian-card-mark">${locked?'???':(evolved?'エボル':'ナビアン')}</span><div class="navian-card-name">${locked?'？？？':name}</div><div class="navian-card-count">${count ? `出会った ${count}回` : 'まだ出会っていない'}</div></div></article>`; }).join('')}</div></div>`;
